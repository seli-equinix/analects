...
class TestUserPreference:
    ...
    def test_update_preference(self, cca, trace_test):
        ...
        cca.chat(
            f"I'm {name}. Identify me and set my verbosity preference "
            f"to 'brief' using update_user_preference.",
            session_id=session_id,
        )

        # Update preference
        result = cca.chat(
            "Change my verbosity preference to 'detailed' using "
        ...
        assert result.content, "Agent returned empty response"
        content_lower = result.content.lower()
        assert "detailed" in content_lower or "updated" in content_lower or \
            "changed" in content_lower or "preference" in content_lower or \
            "ok" in content_lower or "done" in content_lower or \
            "set" in content_lower, \
            "Response doesn't acknowledge the preference update"
