...
class TestGetContext:
    ...
    def test_context_identified_user(self, cca, trace_test, judge_model):
        ...
        try:
            ...
            assert user is not None, \
                f"User '{name}' not created after first message"
            ...
            trace_test.set_attribute(
                "cca.test.user_identified", r2.user_identified,
            )
    ...
    def test_context_shows_stored_facts(self, cca, trace_test, judge_model):
        ...
        try:
            ...
            assert matches >= 2, (
                f"Expected at least 2 stored context terms, "
                f"found {matches}: {result.content[:300]}"
            )
        finally:
            cca.cleanup_test_user(name)
    ...
    def test_context_enriches_responses(self, cca, trace_test, judge_model):
        ...
        try:
            ...
            assert context_used, (
                "Response doesn't reference stored infrastructure "
                f"context: {result.content[:300]}"
            )
        finally:
            cca.cleanup_test_user(name)
