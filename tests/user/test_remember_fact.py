...
Validates that the agent stores facts about users that persist
across sessions. Uses REST API for ground-truth validation.
"""
...
class TestRememberFact:
    ...
    def test_remember_single_fact(self, cca, trace_test):
        ...
        result = cca.chat(
            f"Hi I'm {name}. I work at AcmeCorp — please remember that.",
    ...
    def test_remember_multiple_facts(self, cca, trace_test):
        """Agent should handle multiple facts in one conversation."""
        name = f"MultiFact_{uuid.uuid4().hex[:6]}"
        session_id = f"test-mfact-{uuid.uuid4().hex[:8]}"

        result = cca.chat(
            f"Hi, I'm {name}. Some things about me: I use Python, "
            f"my server is gpu-box, and I work in machine learning. "
            f"Please remember all of that.",
            session_id=session_id,
            timeout=240,
        )
        ...
        # User should exist with profile data
        user = cca.find_user_by_name(name)
        assert user is not None, f"User '{name}' not found via /users API"
    ...
    def test_fact_persists_across_sessions(self, cca, trace_test):
        ...
        cca.chat(
            f"Hi I'm {name}. I work at {company}. Remember that for me.",
            session_id=sid1,
        )

        # Session 2: new session, ask what the agent knows
        result = cca.chat(
            f"Hey, it's {name} again. Where do I work?",
            session_id=sid2,
        )
